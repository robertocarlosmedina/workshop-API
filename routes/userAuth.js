...
router.post('/create', express.json(), async (req, res) => {
	...
	const userAlredyExit = Validation.checkIfUserAlreadyExists(name, email, all_users);

	if(userAlredyExit || name === "" || email === ""){
		return res.sendStatus(401);
	}

	const new_user = await Workshop.postCoordinators(name, email, buffer.from(password).toString('base64'));
