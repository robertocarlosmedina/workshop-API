...
router.get("/", express.json(), async (req, res) => {
  ...
  router.post("/create_team", express.json(), async (req, res) => {
    ...
    if (invalidTeam) return res.sendStatus(500);

    Workshop.postNewTeam(new_team);

    return res.json(new_team);
  });

  
  router.post("/grade_team", express.json(), async (req, res) => {
    const {team_name, coordinator_name, code_readability_grade,
           algorithm_efficiency_grade, completed_tasks_grade,
            creativity_grade,result_analisys_grade} = req.body;
    
    const id_team = await Auxiliar.getTeamIdFromName(team_name);
    const id_coordinator = await Auxiliar.getCoordinatorIdFromName(coordinator_name);
